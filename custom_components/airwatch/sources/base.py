"""Source-layer primitives shared by all AirWatch data sources.
...
Ported ~as-is from PollenWatch ``sources/base.py`` — the Protocol, dataclasses,
exception hierarchy and haversine helper are domain-agnostic. The only domain
change is allergens → pollutants: ``AllergenSeries`` → :class:`PollutantSeries`,
``PollenSource`` → :class:`AirQualitySource`, and the canonical key tuple.
"""
...
# Canonical pollutant keys understood by the source layer. Individual sources
# expose a subset of these. The analytics layer reconciles sources by this key.
#
# These are the literal Open-Meteo air-quality ``hourly`` variable names, so the
# primary source's key→API-var mapping is the identity (no suffix translation,
# unlike PollenWatch's ``{allergen}_pollen``). ``european_aqi`` is an index, not
# a µg/m³ concentration — it rides the same series machinery but the registry /
# sensor layer special-cases its device_class + (absent) unit. See
# ``pollutant_registry.py``.
...
class PollutantSeries:
    """A single pollutant's values for one source result.
    ...
    forecast horizon). A genuine zero reading is ``0.0``, not ``None``.
    ...
    # Source's native categorical value, when it isn't a plain number (e.g. a
    # daily-mean station feed that reports a qualitative band). Carried so
    # analytics can map by the native scale while the raw sensor still shows a
    # numeric value. None for numeric sources.
...
class SourceResult:
    ...
    pollutant in :attr:`pollutants` carries a ``values`` list aligned to it.
    """
    ...
    pollutants: dict[str, PollutantSeries] = field(default_factory=dict)
    generated_at: str | None = None
    message: str | None = None
    # Station-based sources (Sensor.Community nearest sensor, Land Steiermark
    # station) report the picked station here for display. ``snapped_lat/lon``
    # carry the station's coordinates so ``coordinate_shift_km`` is the distance
    # to it.
    station: str | None = None

    @property
    def ok(self) -> bool:
        """True when usable pollutant data was returned."""
...
class AirQualitySource(Protocol):
    ...
    - ``provides_history_series`` — does :meth:`async_fetch` return its own
      ...
      series (Open-Meteo's 92-day backfill). ``False`` (but ``supports_history``)
      → baseline on Home Assistant recorder history.
    """
